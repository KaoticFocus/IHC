...
/**
 * Project Management Service using Supabase for mobile
...
class ProjectManagementService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.error('Error initializing ProjectManagementService:', error);
    }
  ...
  async uploadDocument(
    ...
    const storageFileName = `${userId}/${projectId}/${Date.now()}_${fileName}`;
    ...
    if (docError) {
      ...
      await this.supabase.storage.from('project-files').remove([storageFileName]);
      throw docError;
    }
